...
const API_URL = 'https://healthifyme-o9qv.onrender.com/api';
...
export const registerFCMToken = async () => {
  try {
    ...
    if (!userId) {
      console.warn('[Notification] NO_USER_ID - Make sure userId is set in AsyncStorage after login.');
    ...
    console.log('[Notification] TOKEN_RECEIVED:', { token: token ? 'exists' : 'null' });
    
    if (token) {
      // Always try to register the token with the server
      console.log('[Notification] SENDING_TOKEN_TO_SERVER');      try {
        ...
        if (!response.ok) {
          console.error('[Notification] SERVER_REGISTRATION_FAILED:', await response.text());
...
export const validateStoredToken = async () => {
  try {
    ...
    if (!storedToken || !storedUserId || storedUserId !== currentUserId) {
      console.log('[Notification] TOKEN_INVALID: User mismatch or missing token');
      return false;
    }
