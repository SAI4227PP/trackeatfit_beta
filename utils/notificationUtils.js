...
const logEvent = async (event, details) => {
  ...
  try {
    ...
    await AsyncStorage.setItem(NOTIFICATION_LOGS_KEY, JSON.stringify(parsedLogs.slice(0, 100)));
...
export const storeNotification = async (notification) => {
  try {
    ...
    await AsyncStorage.setItem(NOTIFICATION_LOGS_KEY, JSON.stringify(trimmedLogs));
...
export const requestNotificationPermission = async () => {
  try {
    ...
    await logEvent('PERMISSION_REQUEST', { platform: Platform.OS, permissionGranted });
...
const formatAchievementNotification = (data) => {
  const { achievementTitle, achievementType, achievementDescription, icon } = data;
  return {
...
export const displayLocalNotification = async ({ title, body, data = {}, androidChannelId = 'default' }) => {
  try {
...
const getFriendDataFromChatId = async (chatId) => {
  try {
    ...
    const response = await fetch(`https://healthifyme-o9qv.onrender.com/api/chats/${chatId}`, {
      headers: {
...
export const setupBackgroundHandler = () => {
  ...
  console.log('Background handler will be implemented with a different service');
};
 ...
 */
export const createNotificationSettingsIfNotExists = async (userId, defaultNotificationData) => {
  try {
    // Check if doc exists
    const response = await fetch(`https://healthifyme-o9qv.onrender.com/api/notification-settings?userId=${userId}`, {
      method: 'GET',
      headers: {
    ...
    if (response.status === 404) {
      ...
      const createRes = await fetch('https://healthifyme-o9qv.onrender.com/api/notification-settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId,
          ...Object.fromEntries(
            Object.entries(defaultNotificationData).map(([category, items]) => [
              category,
              Object.fromEntries(
      ...
      if (!createRes.ok) throw new Error('Failed to create notification settings');
      return true;
    }
    // Already exists
    return true;
