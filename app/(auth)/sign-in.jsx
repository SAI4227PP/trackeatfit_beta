...
const SignIn = () => {
  ...
  const getDeviceInfo = useCallback(async () => {
    ...
    const deviceInfo = {
      ...
      browser: Platform.select({
        web: navigator?.userAgent?.includes("Chrome")
          ? "Chrome"
  ...
  const submit = async () => {
    ...
    try {
      ...
      console.log("Sending sign-in request with device info:", {
        email: form.email,
        deviceInfo,
      });
      ...
      if (signInData.token) {
        ...
        if (userData.emailVerification && userData.emailVerification === true) {
          setAlertMessage(
            "Sign-in successful. Your email is already verified.",
          );
          setIsAlertVisible(true);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (signInData.error) {
        if (signInData.error.includes("Account not found")) {
          setAlertMessage("Please sign up first to create your account");
          setIsAlertVisible(true);
          analyticsService.logEvent("login", {
            method: "google",
            status: "error",
      ...
      if (signInData.token && signInData.user) {
        ...
        // Fetch current user data after successful Google sign-in
        const userResponse = await fetch(`${API_URL}/users/get-current-user`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${signInData.token}`,
        ...
        // Update the user data and store it
        await AsyncStorage.setItem("user", JSON.stringify(userData.user));
        setUser(userData.user);
        setIsLoggedIn(true);
        ...
        analyticsService.logEvent("login", {
          method: "google",
          status: "success",
          authMethods: signInData.authMethods,
        });
